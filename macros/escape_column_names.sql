...
{# Different platforms use different escape characters, the default below is for Snowflake which uses double quotes #}
    ...
    {%- if dbtvault.is_something(columns) -%}
        ...
        {%- if columns is string -%}
            ...
            {%- set col_string = escape_column_name(columns) -%}
        ...
        {%- elif dbtvault.is_list(columns) -%}
            ...
            {%- for col in columns -%}
                ...
                {%- if col is string -%}
                    ...
                    {%- set escaped_col = escape_column_name(col) -%}
        ...
        {%- elif columns is mapping -%}
            ...
            {%- if columns['source_column'] and columns['alias'] -%}
                ...
                {%- set escaped_source_col = escape_column_name(columns['source_column']) -%}
                {%- set escaped_alias_col = escape_column_name(columns['alias']) -%}
...
{%- macro escape_column_name(column) -%}
    ...
    {{- adapter.dispatch('escape_column_name')(column=column) -}}
