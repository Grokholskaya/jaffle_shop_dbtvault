...
{# TODO Should the length of the ghost_pk zero hash be determined by the hashing option being used, i.e. MD5 = 16, SHA = 32 ? #}
    ...
    {{- adapter.dispatch('pit')(source_model=source_model, src_pk=src_pk,
...
{%- set src_pk = escape_column_names(src_pk) -%}
{%- set src_ldts = escape_column_names(src_ldts) -%}
{%- set is_any_incremental = false -%}
...
backfill AS (
    ...
    {%- for sat_name in satellites -%}
        ...
        CONVERT(BINARY(16), '{{ ghost_pk }}', 2) AS {{ escape_column_names( sat_name ~ '_' ~ sat_pk_name ) }},
        {%- else -%}
        CAST('{{ ghost_pk }}' AS BINARY(16)) AS {{ escape_column_names( sat_name ~ '_' ~ sat_pk_name ) }},
        {%- endif -%}
        CAST('{{ ghost_date }}' AS {{ type_timestamp() }}) AS {{ escape_column_names( sat_name ~ '_' ~ sat_ldts_name ) }}
        {{- ',' if not loop.last -}}
    {%- endfor %}
    ...
    {% for sat_name in satellites -%}
        ...
        {%- set sat_pk = escape_column_names(satellites[sat_name]['pk'][sat_pk_name]) -%}
        {%- set sat_ldts = escape_column_names(satellites[sat_name]['ldts'][sat_ldts_name]) -%}
        LEFT JOIN {{ ref(sat_name) }} AS {{ escape_column_names( sat_name | lower ~ '_src' ) }}
        {{ "ON" | indent(4) }} a.{{ src_pk }} = {{ escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_pk }}
        {{ "AND" | indent(4) }} {{ escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_ldts }} <= a.AS_OF_DATE
    {% endfor -%}
...
new_rows AS (
    ...
    {%- for sat_name in satellites -%}
        ...
        {%- set sat_ldts = escape_column_names(satellites[sat_name]['ldts'][sat_ldts_name]) %}
        {%- if target.type == "sqlserver" -%}
        COALESCE(MAX({{ escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_pk }}), CONVERT(BINARY(16), '{{ ghost_pk }}', 2)) AS {{ escape_column_names( sat_name | upper ~ '_' ~ sat_pk_name | upper ) }},
        {%- elif target.type == "postgres" -%}
        MAX({{ escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_pk }}) AS {{ escape_column_names( sat_name  ~ '_' ~ sat_pk_name | lower) }},
        {%- else -%}
        COALESCE(MAX({{ escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_pk }}), CAST('{{ ghost_pk }}' AS BINARY(16))) AS {{ escape_column_names( sat_name | upper ~ '_' ~ sat_pk_name | upper ) }},
        {%- endif -%}
        {%- if target.type == "postgres" -%}
        MAX({{ escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_ldts }})  AS {{ escape_column_names( sat_name  ~ '_' ~ sat_ldts_name | lower ) }}
        {%- else -%}
        MAX({{ escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_ldts }})  AS {{ escape_column_names( sat_name  ~ '_' ~ sat_ldts_name ) }}
        {%- endif -%}
        {{- "," if not loop.last }}
    {%- endfor %}
